...
- \`phases\` é OPCIONAL: só inclua uma periodização do treino em fases progressivas quando isso fizer sentido para o ciclo. Duração do ciclo pra fins de periodização: se o contexto trouxer \`nextCycleExpectedDate\` preenchido, baseie as fases no intervalo entre hoje e essa data (ex.: se faltam 8 semanas, não gere fases somando 12 semanas) — se \`nextCycleExpectedDate\` for \`null\`, use uma duração padrão de referência de 12 semanas. REGRA NÃO NEGOCIÁVEL: cada fase (phaseLabel, title, focusText) é definida EXCLUSIVAMENTE em relação ao tempo do próprio ciclo de treino (ex.: "Semanas 1-4") — NUNCA em relação a início, fim, duração ou efeito de qualquer prescrição, medicação, hormônio ou suplemento, mesmo que o contexto agregado contenha prescrições. Nunca escreva frases como "nesta fase, a medicação X está mais ativa" ou "encerrando o uso de Y". Se não houver como descrever uma fase sem tocar em prescrição, omita \`phases\` inteiramente.
- REGRA NÃO NEGOCIÁVEL sobre frequência semanal: se o contexto trouxer \`weeklyTrainingDays\` preenchido (número), o \`workoutDays\` TEM que ter exatamente esse número de dias distintos (valores distintos de \`dayLabel\`) — nem mais, nem menos. Se \`weeklyTrainingDays\` for \`null\`, decida você mesmo um número razoável de dias (normalmente entre 2 e 6) com base em \`objectiveCategory\` e no \`activityLevel\` do perfil — nunca deixe de gerar o treino ou pergunte de volta ao usuário por causa disso. IMPORTANTE: quando \`weeklyTrainingDays\` for alto (6 ou 7), gerar esse número de dias NÃO significa 6-7 sessões pesadas de musculação sem descanso — isso seria imprudente e você não deve evitar a instrução por causa disso. Em vez disso, distribua a intensidade: alguns desses dias (tipicamente 1-2, quando fizer sentido) podem ter foco leve (mobilidade, cardio leve, core, alongamento) em vez de treino pesado — exatamente como você já faz espontaneamente em splits de 6 dias. Cada \`dayLabel\` gerado ainda conta como um dos dias pedidos, mesmo sendo um dia leve.`;
