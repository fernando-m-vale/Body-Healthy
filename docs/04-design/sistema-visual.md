...
**Status:** Rascunho v10 · **Data:** 22/09/2026
...
| Barra de descanso — persistente, não-bloqueante (Spec 09) | **Substitui a ideia inicial de bottom sheet bloqueante** (decisão do Fernando, 08/09/2026: durante o descanso o usuário deve poder continuar vendo/rolando os outros exercícios). Card ancorado na base da tela da sessão ao vivo (não cobre a tela, não esmaece o fundo): contagem em destaque (Space Grotesk, cor `--pulse`) à esquerda, label "Descanso" + próxima série ao centro, indicador de expandir e botão "Pular" (Vital) à direita. Fundo `--ink`, cantos arredondados, flutua sobre o conteúdo com sombra leve. Nunca navega pra tela nova nem impede toque nos elementos por trás. **Decisão confirmada, 10/09/2026:** ao chegar a zero, a barra não fica travada mostrando `00:00` com "Pular" — desaparece sozinha poucos segundos depois do sinal sonoro/vibração, sem exigir toque do usuário |
| Botão de descartar sessão (Spec 09) | **Novo, 10/09/2026:** link de texto "Descartar" na barra superior escura da sessão ao vivo, junto do botão "Concluir" — ação secundária, cor `#B7C4BE` (mesmo tom do label do cronômetro), nunca escondido em menu. Ao tocar, abre um diálogo de confirmação simples (título + duas opções: "Cancelar" / "Descartar") antes de encerrar a sessão sem salvá-la como concluída — ver Spec 09 seção 5.9 |
...
| Gráfico de progressão do dia (Spec 09) | **Novo, 22/09/2026, origem: benchmark Hevy (`claude/10-benchmark-hevy-oportunidades.md`).** Card de superfície (`--mist`, sem sombra) no topo da pré-visualização do dia (tela 30), acima da lista de exercícios. Segmented control com três opções — "Volume", "Repetições", "Duração" — estado ativo em Vital, mesma regra do componente Segmented control já existente. Abaixo do seletor, um gráfico de linha simples (SVG inline, traço `--vital`, sem grid decorativo, eixo com poucos rótulos) mostrando a métrica escolhida ao longo das sessões concluídas anteriores do mesmo dia. Estado vazio (nenhuma sessão anterior): substitui o gráfico por um ícone simples + texto "--muted" "Ainda sem dados" centralizado, mesma altura do componente preenchido, para não pular o layout ao ganhar dado depois. Nunca editável — é só leitura, mesmo espírito da pré-visualização |
| Notificação de descanso com ações rápidas (Spec 09 — evolução futura, sem mockup ainda) | **Registrado, 22/09/2026, origem: benchmark Hevy.** Direção confirmada de produto — a notificação nativa da seção "Barra de descanso" passa a incluir botões de ação diretos ("Concluir série", "Pular", `-15s`/`+15s`) sem abrir o app. Como é uma notificação do sistema operacional, não uma tela do produto, não tem mockup em `mockups.html` — o visual é regido pelas convenções nativas do Android/iOS, não por este guia. Bloqueado até o bug de som/vibração da notificação atual ser resolvido; não implementar antes disso |
...
| 30 | Pré-visualização do dia (antes de iniciar) | Spec 09 | Desenhada — nova (10/09/2026): tocar num dia na tela 27 abre esta lista somente-leitura dos exercícios antes de criar a sessão; botão "Iniciar treino" no rodapé é quem de fato inicia. Atualizada (22/09/2026): ganhou o gráfico de progressão do dia no topo (Volume/Repetições/Duração), origem benchmark Hevy — ver seção 5 |
